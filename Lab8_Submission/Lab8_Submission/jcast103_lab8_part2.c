...
enum SCALE_STATE { SMStart1, Wait, Press1, Press2, ScaleU, ScaleD } state2;

enum SOUND_STATE { SMStart2, Off, Pressed1, On, Pressed2 } state1;

double noiseAry[8] = { 261.63, 293.66, 329.63, 349.23, 392.00, 440.00, 493.88, 523.25 };
...
void Scale(){
	...
	//A1 controls scaling up, A2 controls scaling down
        switch(state2){
		case SMStart1: 
			state2 = Wait;
			break;
		...
		case Wait:
			if(A1 && !A2){ state2 = Press1; }
			else if(A2 && !A1) { state2 = Press2; }
			break;
	...
	switch(state2){
		...
		case ScaleU:
			if(index == 7 && on){
				set_PWM(noiseAry[index]);
			}
			else if(index < 7 && index >= 0 && on){ 
				index++;
				set_PWM(noiseAry[index]);
			}
			break;

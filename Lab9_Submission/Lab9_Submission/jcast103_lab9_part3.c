...
enum SPEAK_STATES { S_SMStart, OffS, Wait1, OnS, Wait2 } state3;
...
void SpeakerSM(){
	...
	switch(state3){
		...
		case OnS:
			//Making the speaker turn on and off every 2 ms
			if(counter <= 2){
				speaker = 0x01;
			}
			else if(counter <= 4){
...
void CombineLEDsSM(){
	...
	temp = threeLEDs | (blinkingLED << 3) | (speaker << 4);
	PORTB = temp;
...
int main(void) {
	...
	DDRA = 0x00; PORTA = 0xFF; //Setting port A to input
	DDRB = 0xFF; PORTB = 0x00; //Setting port B to output
	/* Insert your solution below */
	unsigned short tickCnt1 = 0;
	unsigned short tickCnt2 = 0;
	unsigned short tickCnt3 = 0;
	TimerSet(1);
	TimerOn();
